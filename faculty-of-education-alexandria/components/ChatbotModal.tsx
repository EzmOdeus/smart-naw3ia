...
const ChatbotModal: React.FC<ChatbotModalProps> = ({ visible, onClose }) => {
  ...
  const constructPrompt = (userInput: string): string => {
    ...
    مع تجنب أي معلومات غير ذات صلة أو خارج نطاق تخصصك.
    إذا لم تكن متأكدًا من الإجابة، يمكنك الرد بأنك ستحتاج إلى التحقق من المعلومات أو توجيه المستخدم إلى مصدر موثوق.
    تأكد من أن إجابتك تتضمن معلومات دقيقة ومحدثة عن الكلية.
    إذا كان هناك أي معلومات إضافية قد تكون مفيدة للمستخدم، فلا تتردد في تضمينها.
    تذكر أن هدفك هو توفير أفضل مساعدة ممكنة للمستخدم فيما يتعلق بكلية التربية النوعية بجامعة الإسكندرية.
`;
  };
